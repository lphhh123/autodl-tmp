...
class UCF101Dataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        video = torch.stack(tensor_frames, dim=0).clone()  # [T, C, H, W]
        ...
        if audio is not None:
            sample["audio"] = audio.clone()
        return sample


def _pad_or_trim_video(video: torch.Tensor, target_len: int) -> torch.Tensor:
    if video.shape[0] > target_len:
        return video[:target_len]
    if video.shape[0] < target_len:
        pad = video[-1:].repeat(target_len - video.shape[0], 1, 1, 1)
        return torch.cat([video, pad], dim=0)
    return video


def _pad_or_trim_audio(audio: torch.Tensor, target_len: int) -> torch.Tensor:
    if audio.shape[0] > target_len:
        return audio[:target_len]
    if audio.shape[0] < target_len:
        pad = audio[-1:].repeat(target_len - audio.shape[0], 1)
        return torch.cat([audio, pad], dim=0)
    return audio


def collate_ucf101(batch: List[Dict[str, torch.Tensor]], target_len: int | None = None) -> Dict[str, torch.Tensor]:
    if target_len is None:
        target_len = batch[0]["video"].shape[0]
    videos = torch.stack([_pad_or_trim_video(item["video"].clone(), target_len) for item in batch], dim=0)
    labels = torch.stack([item["label"] for item in batch], dim=0)
    out = {"video": videos, "label": labels}
    if "audio" in batch[0]:
        out["audio"] = torch.stack([_pad_or_trim_audio(item["audio"].clone(), target_len) for item in batch], dim=0)
    return out
