...
def _canonicalize_traffic_to_SxS(traffic: np.ndarray, mapping: List[int], S: int):
    ...
    if traffic.shape == (S, S):
        ...
        return traffic, {
            ...
            "traffic_compact_shape": list(traffic.shape),
            "used_slots": used_slots,
            "traffic_slot_ids": used_slots,
...
def _export_layout_input(
    ...
    traffic_full_t, traffic_meta_solver = mapping_solver.build_traffic_matrix(
        segments, mapping, num_slots=S, return_meta=True
    )
    traffic_np = traffic_full_t.detach().cpu().numpy().astype(float)
    traffic, traffic_meta_canon = _canonicalize_traffic_to_SxS(traffic_np, mapping, S)
    traffic_meta = {**traffic_meta_solver, **traffic_meta_canon}
    ...
    layout_input = {
        ...
        "mapping": {
            ...
            "mapping": mapping,
            "used_slots": traffic_meta.get("used_slots", []),
            "traffic_mode": traffic_meta.get("traffic_mode", "full_SxS_slot_order_0_to_S-1"),
            "traffic_shape_in": tuple(int(x) for x in traffic_meta.get("traffic_shape_in", (S, S))),
            "S": int(traffic_meta.get("S", S)),
            "U": int(traffic_meta.get("U", S)),
            "meta": traffic_meta,
        },
        "baseline": baseline,
