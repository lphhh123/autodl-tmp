...
def _export_layout_input(
    ...
    traffic = mapping_solver.build_traffic_matrix(segments, mapping, S=S, mode="full").cpu().numpy().astype(float)
    if traffic.shape != (S, S) or not np.all(np.isfinite(traffic)):
        raise ValueError(
            f"[export_layout_input] traffic_matrix invalid shape {traffic.shape} for S={S} or contains non-finite values."
        )
