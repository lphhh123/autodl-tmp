...
def _build_state_and_evaluator(
    ...
    assign_grid = np.asarray(baseline.get("assign_grid", _derive_initial_assign(layout_input)), dtype=int)
    baseline_used, _baseline_meta = _prepare_baseline_for_run(
        layout_input=layout_input,
        sites_xy=sites_xy,
        assign_grid=assign_grid,
        chip_tdp=tdp,
        traffic=traffic,
        sigma_target=float(objective_cfg["sigma_mm"]),
...
def main() -> None:
    ...
    # ---- Objective scalar weight overrides (for sweeps; keep fairness with run_layout_agent) ----
    ...
    objective_cfg_guard = _build_objective_cfg(cfg)
    baseline_policy = str(os.environ.get("LAYOUT_BASELINE_POLICY", "auto")).strip().lower()
    _sites_xy_for_base = np.asarray(layout_input.get("sites", {}).get("sites_xy", []), dtype=np.float32)
    _assign_grid_for_base = np.asarray(
        layout_input.get("baseline", {}).get("assign_grid", _derive_initial_assign(layout_input)),
        dtype=int,
    )
    _chip_tdp_for_base = np.asarray(layout_input.get("slots", {}).get("tdp", []), dtype=float)
    _traffic_for_base = np.asarray(layout_input.get("mapping", {}).get("traffic_matrix", []), dtype=float)
    baseline_used_for_meta, baseline_guard_meta = _prepare_baseline_for_run(
        layout_input=layout_input,
        sites_xy=_sites_xy_for_base,
        assign_grid=_assign_grid_for_base,
        chip_tdp=_chip_tdp_for_base,
        traffic=_traffic_for_base,
        sigma_target=float(objective_cfg_guard["sigma_mm"]),
        policy=baseline_policy,
    )
    ...
    try:
        ...
        init_trace_dir_v54(
            ...
            run_meta={
                ...
                "baseline_guard": baseline_guard_meta,
                "baseline_used": baseline_used_for_meta,
            },
