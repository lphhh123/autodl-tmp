...
def run_layout_agent(cfg, out_dir: Path, seed: int, layout_input_path: str | Path | None = None) -> None:
    ...
    ensure_trace_file(
        ...
        header={
            ...
            "steps_planned": int(detailed_cfg.get("steps", 0) or detailed_cfg.get("max_steps", 0) or 0),
            "signature": sig,
        },
    )
    ...
    try:
        ...
        if "assign_final" in locals() and assign_final is not None:
            final_payload.setdefault("layout_signature", signature_from_assign(assign_final))
        final_payload.setdefault("mapping_signature", "")
        final_payload.setdefault("trace_csv", str((out_dir / "trace.csv").name))
        finalize_trace(str(trace_jsonl), final_payload, status=status)
