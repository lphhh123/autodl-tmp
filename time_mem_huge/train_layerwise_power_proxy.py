# train_layerwise_power_proxy.py
import argparse
import csv
import json
import math
import random
from collections import defaultdict

import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim
import yaml

PATCH_SIZE = 16
IN_CHANS = 3


# ---------- 基本 FLOPs / Bytes 估计（和之前保持一致） ---------- #

def gemm_flops_bytes(M, K, N, bpe):
    flops = 2.0 * M * K * N
    bytes_ = bpe * (M * K + K * N + M * N)
    return flops, bytes_


def attn_core_flops_bytes(L, dmodel, dk, dv, heads, bpe):
    H = heads if heads is not None else max(1, dmodel // dk)
    flops = 2.0 * H * (L ** 2) * (dk + dv)

    bytes_qkv = 3.0 * L * H * dk
    bytes_out = 1.0 * L * H * dv
    bytes_attn = 2.0 * (L ** 2) * H
    bytes_ = bpe * (bytes_qkv + bytes_out + bytes_attn)
    return flops, bytes_


def flops_bytes_layer(row, bpe):
    """
    根据 layer_type / embed_dim / num_heads / mlp_ratio / L_patch / L_eff / bs
    估算该层的 FLOPs_L / Bytes_L。

    layer_type：
      - patch_embed: 一个 GEMM（conv unfold + matmul 近似）
      - attn: qkv GEMM + attn_core + proj GEMM
      - mlp: 两个 GEMM（fc1 + fc2）
    """
    layer_type = row["layer_type"]
    bs = row["bs"]
    embed_dim = row["embed_dim"]
    num_heads = max(1, row["num_heads"])
    mlp_ratio = row["mlp_ratio"]
    L_patch = row["L_patch"]
    L_eff = row["L_eff"]

    if layer_type == "patch_embed":
        M = L_patch * bs
        K = IN_CHANS * PATCH_SIZE * PATCH_SIZE
        N = embed_dim
        F, B = gemm_flops_bytes(M, K, N, bpe)

    elif layer_type == "attn":
        head_dim = embed_dim // num_heads
        dk = dv = head_dim

        # qkv: [B*L, C] x [C, 3C]
        M_qkv = L_eff * bs
        K_qkv = embed_dim
        N_qkv = 3 * embed_dim
        F_qkv, B_qkv = gemm_flops_bytes(M_qkv, K_qkv, N_qkv, bpe)

        # attn_core
        F_attn, B_attn = attn_core_flops_bytes(
            L=L_eff, dmodel=embed_dim, dk=dk, dv=dv,
            heads=num_heads, bpe=bpe
        )
        F_attn *= bs
        B_attn *= bs

        # proj: [B*L, C] x [C, C]
        M_proj = L_eff * bs
        K_proj = embed_dim
        N_proj = embed_dim
        F_proj, B_proj = gemm_flops_bytes(M_proj, K_proj, N_proj, bpe)

        F = F_qkv + F_attn + F_proj
        B = B_qkv + B_attn + B_proj

    else:  # mlp
        d_ff = int(embed_dim * mlp_ratio)
        M = L_eff * bs

        # fc1
        F1, B1 = gemm_flops_bytes(M, embed_dim, d_ff, bpe)
        # fc2
        F2, B2 = gemm_flops_bytes(M, d_ff, embed_dim, bpe)
        F = F1 + F2
        B = B1 + B2

    return F, B


# ---------- load gpu_data / power_dataset ---------- #

def load_gpu_data(yaml_path):
    with open(yaml_path) as f:
        data = yaml.safe_load(f)

    chip_map = {}
    for chip in data.get("chiplets", []):
        name = chip["name"]  # e.g. RTX2080Ti_FP16
        chip_map[name] = chip

    defaults = data.get("defaults", {})
    return chip_map, defaults


def load_power_dataset(path):
    rows = []
    with open(path) as f:
        r = csv.DictReader(f)
        for row in r:
            rows.append(
                {
                    "device": row["device"],
                    "cfg": row["cfg"],
                    "layer_type": row["layer_type"],
                    "prec": row["prec"],  # 'fp16' / 'fp32'
                    "img": int(row["img"]),
                    "bs": int(row["bs"]),
                    "keep_ratio": float(row["keep_ratio"]),
                    "L_patch": int(row["L_patch"]),
                    "L_eff": int(row["L_eff"]),
                    "avg_power_w": row["avg_power_w"],
                    "energy_mj": row["energy_mj"],
                    "ms_event": row["ms_event"],
                    "status": row["status"],
                    "depth": int(row["depth"]),
                    "embed_dim": int(row["embed_dim"]),
                    "num_heads": int(row["num_heads"]),
                    "mlp_ratio": float(row["mlp_ratio"]),
                    "complexity_ratio": float(row["complexity_ratio"]),
                    "head_dim": int(row["head_dim"]),
                    "tp_world_size": int(row["tp_world_size"]),
                }
            )
    return rows


# ---------- 特征构造（和 ms/mem 用同一套） ---------- #

def build_features(rows, chip_map, defaults):
    feats = []
    y_power = []
    meta = []

    eta_comp_default = float(defaults.get("eta_comp", 0.75))
    eta_bw_default = float(defaults.get("eta_bw", 0.75))

    for row in rows:
        if row["status"] != "ok":
            continue
        if row["avg_power_w"] is None or row["avg_power_w"] == "":
            continue

        layer_type = row["layer_type"]
        prec = row["prec"].lower()
        if prec == "fp16":
            prec_tag = "FP16"
            bpe = 2
        else:
            prec_tag = "FP32"
            bpe = 4

        chip_key = f"{row['device']}_{prec_tag}"
        chip = chip_map.get(chip_key, None)
        if chip is None:
            print(f"[warn] skip sample: unknown chip {chip_key}")
            continue

        peak_flops = float(chip["peak_flops"])
        peak_bw = float(chip["peak_bw"])
        eta_comp = eta_comp_default
        eta_bw = eta_bw_default

        # per-layer FLOPs / Bytes
        F_L, B_L = flops_bytes_layer(row, bpe)

        # roofline 下界
        t_comp = F_L / (eta_comp * peak_flops + 1e-9)
        t_bw = B_L / (eta_bw * peak_bw + 1e-9)
        t_roof = max(t_comp, t_bw)

        # layer_type one-hot
        if layer_type == "patch_embed":
            lt = [1.0, 0.0, 0.0]
        elif layer_type == "attn":
            lt = [0.0, 1.0, 0.0]
        else:
            lt = [0.0, 0.0, 1.0]

        img = row["img"]
        bs = row["bs"]
        keep = row["keep_ratio"]
        L_patch = row["L_patch"]
        L_eff = row["L_eff"]
        embed_dim = row["embed_dim"]
        num_heads = max(1, row["num_heads"])
        mlp_ratio = row["mlp_ratio"]
        head_dim = row["head_dim"]

        x = [
            math.log10(F_L + 1e-9),
            math.log10(B_L + 1e-9),
            math.log10(t_roof + 1e-9),
            t_comp / (t_roof + 1e-9),
            t_bw / (t_roof + 1e-9),

            img / 224.0,
            bs / 64.0,
            keep,

            L_patch / 256.0,
            L_eff / 256.0,

            embed_dim / 1280.0,
            num_heads / 16.0,
            head_dim / 128.0,
            mlp_ratio / 4.0,

            row["complexity_ratio"],

            math.log10(peak_flops + 1e-9),
            math.log10(peak_bw + 1e-9),

            *lt,
        ]

        feats.append(x)
        y_power.append(float(row["avg_power_w"]))

        meta.append({
            "cfg": row["cfg"],
            "prec": row["prec"],
            "device": row["device"],
            "layer_type": layer_type,
        })

    X = np.asarray(feats, dtype=np.float32)
    y_power = np.asarray(y_power, dtype=np.float32)
    return X, y_power, meta


# ---------- train/test 划分（按 cfg） ---------- #

def split_train_test_by_cfg(rows, train_ratio=0.8, seed=42):
    cfgs = sorted({r["cfg"] for r in rows if r["status"] == "ok"})
    random.Random(seed).shuffle(cfgs)
    n_train = int(len(cfgs) * train_ratio)
    train_cfgs = set(cfgs[:n_train])
    test_cfgs = set(cfgs[n_train:])

    train_rows, test_rows = [], []
    for r in rows:
        if r["status"] != "ok":
            continue
        if r["cfg"] in train_cfgs:
            train_rows.append(r)
        elif r["cfg"] in test_cfgs:
            test_rows.append(r)
    return train_rows, test_rows, train_cfgs, test_cfgs


# ---------- 模型 & 训练 ---------- #

class PowerProxyMLP(nn.Module):
    def __init__(self, in_dim, hidden=128):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(in_dim, hidden),
            nn.ReLU(),
            nn.Linear(hidden, hidden),
            nn.ReLU(),
            nn.Linear(hidden, 1),
        )

    def forward(self, x):
        return self.net(x).squeeze(-1)


def mape(pred, gt, eps=1e-6):
    pred = np.asarray(pred, dtype=np.float64)
    gt = np.asarray(gt, dtype=np.float64)
    return float(np.mean(np.abs(pred - gt) / np.maximum(np.abs(gt), eps)))


def mape_group(pred, gt, metas, key_fn):
    buckets = defaultdict(lambda: {"pred": [], "gt": []})
    for p, g, m in zip(pred, gt, metas):
        k = key_fn(m)
        buckets[k]["pred"].append(p)
        buckets[k]["gt"].append(g)

    out = {}
    for k, v in buckets.items():
        out[k] = mape(v["pred"], v["gt"])
    return out


def train_power(model, X_train, y_train, epochs=1200, lr=1e-3, weight_decay=1e-5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)

    X_t = torch.from_numpy(X_train).to(device)
    y_t = torch.from_numpy(y_train).to(device)

    opt = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    loss_fn = nn.MSELoss()

    model.train()
    for e in range(1, epochs + 1):
        opt.zero_grad()
        pred = model(X_t)
        loss = loss_fn(pred, y_t)
        loss.backward()
        opt.step()

        if e % max(1, epochs // 5) == 0:
            print(f"[train-power] epoch {e}/{epochs}, loss={loss.item():.6f}")
    return model


def predict_power(model, X):
    device = next(model.parameters()).device
    X_t = torch.from_numpy(X).to(device)
    model.eval()
    with torch.no_grad():
        y = model(X_t).cpu().numpy()
    return y


# ---------- 主流程 ---------- #

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data", default="layer_power_dataset.csv")
    ap.add_argument("--gpu_yaml", default="gpu_data.yaml")
    ap.add_argument("--out", default="proxy_layer_power_report.json")
    ap.add_argument("--save_model", default="layer_proxy_power.pt")
    args = ap.parse_args()

    # 固定随机种子
    random.seed(42)
    np.random.seed(42)
    torch.manual_seed(42)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(42)

    chip_map, defaults = load_gpu_data(args.gpu_yaml)
    rows = load_power_dataset(args.data)
    print(f"[data] total layer power samples (raw) = {len(rows)}")

    train_rows, test_rows, train_cfgs, test_cfgs = split_train_test_by_cfg(
        rows, train_ratio=0.8
    )
    print(f"[split] train_cfgs = {len(train_cfgs)}, test_cfgs = {len(test_cfgs)}")
    print(f"[split] train_samples(raw) = {len(train_rows)}, test_samples(raw) = {len(test_rows)}")

    X_train, y_train, meta_train = build_features(train_rows, chip_map, defaults)
    X_test, y_test, meta_test = build_features(test_rows, chip_map, defaults)

    print(f"[feat] feature_dim = {X_train.shape[1]}")
    print(f"[feat] train_samples = {X_train.shape[0]}, test_samples = {X_test.shape[0]}")

    model = PowerProxyMLP(in_dim=X_train.shape[1], hidden=128)
    model = train_power(model, X_train, y_train, epochs=1200, lr=1e-3, weight_decay=1e-5)

    pred_train = predict_power(model, X_train)
    pred_test = predict_power(model, X_test)

    train_mape = mape(pred_train, y_train)
    test_mape = mape(pred_test, y_test)

    per_prec_train = mape_group(pred_train, y_train, meta_train, key_fn=lambda m: m["prec"])
    per_prec_test = mape_group(pred_test, y_test, meta_test, key_fn=lambda m: m["prec"])
    per_layer_type_test = mape_group(
        pred_test, y_test, meta_test, key_fn=lambda m: m["layer_type"]
    )

    print(f"[POWER] train MAPE = {train_mape*100:.2f}%")
    print(f"[POWER] test  MAPE = {test_mape*100:.2f}%")

    report = {
        "power": {
            "train_mape": train_mape,
            "test_mape": test_mape,
            "per_prec_train": per_prec_train,
            "per_prec_test": per_prec_test,
            "per_layer_type_test": per_layer_type_test,
        }
    }

    with open(args.out, "w") as f:
        json.dump(report, f, indent=2)
    print(f"\n[done] report written to {args.out}")
    print(json.dumps(report, indent=2))

    torch.save(model.state_dict(), args.save_model)
    print(f"[done] power proxy model saved to {args.save_model}")


if __name__ == "__main__":
    main()
